...
class Database:
    ...
    async def add_file(self, file_info, temp: bool = False):
        ...
        if fetch_old:
            if fetch_old.get("temp", False):
                if current_time - fetch_old.get("time", 0) > 24 * 60 * 60:
    ...
    async def add_premium_user(self, id):
        await self.premium.update_one(
            {'_id': 'premium_users_list'},
            {'$addToSet': {'user_ids': int(id)}},
            upsert=True
        )

    async def is_user_premium(self, id):
        premium_users_doc = await self.premium.find_one({'_id': 'premium_users_list'})
        if premium_users_doc and 'user_ids' in premium_users_doc:
            return int(id) in premium_users_doc['user_ids']
        return False
    ...
    async def update_file_temp_status(self, _id, temp_status: bool):
        try:
            result = await self.file.update_one(
                {"_id": ObjectId(_id)},
                {"$set": {"temp": temp_status}}
            )
    ...
    async def make_file_permanent(self, _id):
        return await self.update_file_temp_status(_id, False)
