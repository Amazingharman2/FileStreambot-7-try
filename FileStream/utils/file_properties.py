...
def get_file_info(message):
    ...
    # Extra MTProto/Telegram-internal fields for JS/gramJS
    # Try to keep this generic for both documents and photos.

    # dc_id exists on most downloadable media
    file_info["dc_id"] = getattr(media, "dc_id", None)

    # Document-specific info (for files, videos, etc.)
    if getattr(message, "document", None):
        doc = message.document
        file_info["document_id"] = getattr(doc, "id", None)
        file_info["document_access_hash"] = getattr(doc, "access_hash", None)
        # raw reference bytes; stored as Binary in Mongo
        file_info["document_file_reference"] = getattr(doc, "file_reference", None)

    # Photo-specific info
    if getattr(message, "photo", None):
        photo = message.photo
        file_info["photo_id"] = getattr(photo, "id", None)
        file_info["photo_access_hash"] = getattr(photo, "access_hash", None)
        file_info["photo_file_reference"] = getattr(photo, "file_reference", None)
