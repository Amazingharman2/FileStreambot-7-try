...
from FileStream import FIleNotFound
...
@FileStream.on_message(filters.command("servers") & filters.private)
async def server_status_handler(client, message):
    health_url = "https://io-filedownloader.vercel.app/health"
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(health_url) as resp:
                if resp.status != 200:
                    return await message.reply_text("⚠️ Failed to fetch server status. Try again later.", quote=True)
    ...
    status_text = "🌐 **Server Status Report**\n\n"

    status_text += "✅ **Healthy Servers:**\n"
    for server in healthy:
        status_text += f"🗸 `{server['name']}`\n"
    status_text += "\n❌ **Unhealthy Servers:**\n"
